...
type Config struct {
	...
	SelReportInterval time.Duration `required:"false" default:"1h" desc:"the interval for periodical system event log reports" split_words:"true"`
...
func (c Config) String() string {
	return fmt.Sprintf("loglevel:%s partition:%s leasefile:%s report interval:%s sel interval:%s metal-api url:%s ipmiport:%d ipmiuser:%s, ignored-macs:%v",
		c.LogLevel, c.PartitionID, c.LeaseFile, c.ReportInterval, c.SelReportInterval, c.MetalAPIURL, c.IpmiPort, c.IpmiUser, c.IgnoreMacs)
}
